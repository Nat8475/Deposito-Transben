...
function enviarEmail(assunto, htmlBody, anexos, tipoAlerta) {
  try {
    ...
    try { opts.inlineImages = { logo_transben: DriveApp.getFileById(ID_LOGO_TRANSBEN).getBlob() }; } catch(_) {}
    if (tipoAlerta) {
...
function _getPageContent(page) {
  ...
  try {
    ...
    var pgKey   = 'pg_html_v12t_' + pagina;
...
function limparCachePaginas() {
  var cache = CacheService.getScriptCache();
  var keys = Object.keys(_WEBAPP_PAGINAS).map(function(p){ return 'pg_html_v12t_' + _WEBAPP_PAGINAS[p]; });
